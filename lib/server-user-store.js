...
export async function readCloudStore() {
    ...
    if (res.ok) {
      ...
      if (contentStr) {
        ...
        if (Array.isArray(parsed.users)) {
          ...
          // Merge disk store fallback if local store has newer or more accounts
          const localStore = readLocalDiskStore();
          let mergedUsers = [...parsed.users];
          if (localStore && Array.isArray(localStore.users)) {
            localStore.users.forEach(lu => {
              const luEmpId = (lu.employeeId || lu.employee_id || lu.id || '').toUpperCase();
              const idx = mergedUsers.findIndex(u => (u.employeeId || u.employee_id || u.id || '').toUpperCase() === luEmpId);
              if (idx === -1) {
                mergedUsers.push(lu);
              } else {
                if (lu.updatedAt && (!mergedUsers[idx].updatedAt || lu.updatedAt > mergedUsers[idx].updatedAt)) {
          ...
          const sanitizedUsers = mergedUsers.map(u => {
            ...
            if (!empId || !empId.startsWith('TH')) {
              empId = generateNextEmployeeId(currentRole, mergedUsers, deleted, activities);
            }
          ...
          const cloudPayload = { users: sanitizedUsers, deleted, activities };
          writeLocalDiskStore(cloudPayload);
          return cloudPayload;
        }
      }
    }
  ...
  // Fallback to local disk store if Frappe Cloud returns 429 or fails
  const localDiskData = readLocalDiskStore();
  if (localDiskData && Array.isArray(localDiskData.users) && localDiskData.users.length > 0) {
    return localDiskData;
  }

  // Seed default accounts if cloud store read returns empty
  const defaultPayload = { users: INITIAL_ACCOUNTS, deleted: [], activities: [] };
  writeLocalDiskStore(defaultPayload);
