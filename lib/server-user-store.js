...
const INITIAL_ACCOUNTS = [
  {
    ...
    sessionVersion: 1,
    createdAt: '2026-08-24T08:18:16.914Z',
    creation: '2026-08-24T08:18:16.914Z',
    joined_date: '2026-08-24',
    date_of_joining: '2026-08-24'
  },
  {
    id: 'STAFF-MGR-1',
    employeeId: 'TH002',
    ...
    sessionVersion: 1,
    createdAt: '2026-08-27T08:39:39.097Z',
    creation: '2026-08-27T08:39:39.097Z',
    joined_date: '2026-08-27',
    date_of_joining: '2026-08-27'
  }
];
...
const LOCAL_STORE_PATH = path.join(process.cwd(), isProductionEnvironment() ? 'server_user_store.json' : 'server_user_store_dev.json');
const TMP_STORE_PATH = path.join(os.tmpdir(), isProductionEnvironment() ? 'server_user_store.json' : 'server_user_store_dev.json');
...
function writeLocalDiskStore(payload) {
  ...
  try {
    ...
    try {
      ...
      console.warn("Local tmp store write error:", tmpErr.message);
    }
  }
}
 ...
 * In Local Development: Reads strictly from isolated dev disk store.
 * In Production: Connects to Frappe Cloud central store.
 */
export async function readCloudStore() {
  ...
  // ONLY sync with Frappe Cloud in Production. In local dev, stay 100% isolated.
  ...
  const sanitizedUsers = activeUsers.map(u => {
    ...
    const rawDate = u.createdAt || u.creation || u.joined_date || u.date_of_joining || u.updatedAt || '2026-08-24T08:18:16.914Z';
...
export async function writeAndVerifyCloudStore(users, deleted, activities = null) {
  ...
  // In Local Development: Store is 100% isolated locally. Do NOT mutate Production Frappe Cloud Note.
  if (!isProductionEnvironment()) {
    return payload;
  }
...
export async function saveServerUser(userData) {
  ...
  const updatedUser = {
    ...
    createdAt: userData.createdAt || (existingUser ? (existingUser.createdAt || existingUser.creation || existingUser.joined_date || existingUser.date_of_joining || existingUser.updatedAt) : new Date().toISOString()),
    creation: userData.creation || userData.createdAt || (existingUser ? (existingUser.creation || existingUser.createdAt || existingUser.joined_date || existingUser.date_of_joining || existingUser.updatedAt) : new Date().toISOString()),
    joined_date: (userData.joined_date || userData.date_of_joining || userData.createdAt || (existingUser ? (existingUser.joined_date || existingUser.date_of_joining || existingUser.createdAt || existingUser.creation) : new Date().toISOString())).split('T')[0],
    date_of_joining: (userData.date_of_joining || userData.joined_date || userData.createdAt || (existingUser ? (existingUser.date_of_joining || existingUser.joined_date || existingUser.createdAt || existingUser.creation) : new Date().toISOString())).split('T')[0],
