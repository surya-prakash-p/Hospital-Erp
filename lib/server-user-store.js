...
const INITIAL_ACCOUNTS = [
  ...
  },
  {
    id: 'DOC-001',
    userId: 'DOC-001',
    email: 'drkumar@thangamhospital.com',
    ...
    role: 'Doctor',
    roles: ['Doctor'],
    permissions: ['consultation', 'patient_registry', 'appointments', 'pharmacy.view'],
  ...
  },
  {
    id: 'DOC-002',
    userId: 'DOC-002',
    email: 'drpriya@thangamhospital.com',
  ...
  },
  {
    id: 'PHARM-001',
    userId: 'PHARM-001',
    email: 'ravi@thangamhospital.com',
    ...
    role: 'Pharmacist',
    roles: ['Pharmacist'],
    permissions: ['pharmacy', 'inventory', 'dispensing', 'batch.manage'],
  ...
  },
  {
    id: 'PHARM-002',
    userId: 'PHARM-002',
    email: 'priya@thangamhospital.com',
...
export async function saveServerUser(userData) {
  ...
  const roles = userData.roles || (existingUser ? existingUser.roles : [userData.role || 'Staff Member']);
  const primaryRole = userData.role || (roles?.[0]) || 'Staff Member';

  // Role-based ID Generation (e.g. DOC-003, PHARM-003, ADMIN-003)
  let generatedUserId = userData.userId || userData.id || (existingUser ? (existingUser.userId || existingUser.id) : null);
  if (!generatedUserId) {
    const rolePrefix = primaryRole === 'Doctor' ? 'DOC' :
                       primaryRole === 'Pharmacist' ? 'PHARM' :
                       (primaryRole === 'Hospital Admin' || primaryRole === 'System Manager') ? 'ADMIN' : 'STAFF';
    const sameRoleCount = users.filter(u => (u.role === primaryRole || u.roles?.includes(primaryRole))).length + 1;
    generatedUserId = `${rolePrefix}-${String(sameRoleCount).padStart(3, '0')}`;
  }

  // Role-based Default Permissions
  let permissions = userData.permissions || (existingUser ? existingUser.permissions : []);
  if (!permissions || permissions.length === 0) {
    if (primaryRole === 'Doctor') {
      permissions = ['consultation', 'patient_registry', 'appointments', 'pharmacy.view'];
    } else if (primaryRole === 'Pharmacist') {
      permissions = ['pharmacy', 'inventory', 'dispensing', 'batch.manage'];
    } else if (primaryRole === 'Hospital Admin' || primaryRole === 'System Manager') {
      permissions = ['*'];
    } else if (primaryRole === 'Nurse') {
      permissions = ['patient_registry', 'ipd', 'appointments'];
    } else if (primaryRole === 'Lab Technician') {
      permissions = ['lab'];
    } else if (primaryRole === 'Receptionist') {
      permissions = ['reception', 'appointments', 'patient_registry'];
    } else if (primaryRole === 'Billing Clerk') {
      permissions = ['billing', 'finance'];
    } else {
      permissions = ['general'];
    }
  }
  ...
  const updatedUser = {
    ...
    name: userData.full_name || (existingUser ? existingUser.full_name : 'Staff Member'),
    role: primaryRole,
    roles: roles,
