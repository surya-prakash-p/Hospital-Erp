...
export async function saveServerUser(userData) {
  ...
  const users = cloudStore.users || readServerUsers();
  const deletedList = cloudStore.deleted || readDeletedUsers();
  ...
  await writeCloudStore(users, deletedList);
  return updatedUser;
}
...
export async function deleteServerUser(identifier) {
  const cloudStore = await readCloudStore();
  let users = cloudStore.users || readServerUsers();
  let deletedList = cloudStore.deleted || readDeletedUsers();
  ...
  await writeCloudStore(filtered, deletedList);
  return true;
}
