...
export async function saveServerUser(userData) {
  ...
  const updatedUser = {
    ...
    createdAt: userData.createdAt || (existingUser ? (existingUser.createdAt || existingUser.creation || existingUser.joined_date || existingUser.updatedAt) : new Date().toISOString()),
    creation: userData.creation || userData.createdAt || (existingUser ? (existingUser.creation || existingUser.createdAt || existingUser.joined_date || existingUser.updatedAt) : new Date().toISOString()),
    joined_date: (userData.joined_date || userData.createdAt || (existingUser ? (existingUser.joined_date || existingUser.createdAt || existingUser.creation) : new Date().toISOString())).split('T')[0],
