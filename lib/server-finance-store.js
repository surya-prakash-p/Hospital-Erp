...
const LOCAL_STORE_PATH = path.join(process.cwd(), isProductionEnvironment() ? "server_finance_store.json" : "server_finance_store_dev.json");
const TMP_STORE_PATH = path.join(os.tmpdir(), isProductionEnvironment() ? "server_finance_store.json" : "server_finance_store_dev.json");
...
export async function writeFinanceStore(transactions) {
  ...
  // In Local Development: Skip cloud write, keep strictly in local dev store.
  if (!isProductionEnvironment()) {
    return payload;
  }
...
export async function addFinanceTransaction(tx) {
  ...
  const isRefund = tx.type === "Refund" || (tx.category || "").toLowerCase().includes("refund") || (tx.category || "").toLowerCase().includes("return") || rawAmt < 0;

  const newEntry = {
    id: tx.id || `tx-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
    title: tx.title || (isRefund ? "Medicine Return Refund" : "Hospital Transaction"),
    type: isRefund ? "Refund" : (tx.type || "Expense"),
    category: tx.category || (isRefund ? "Pharmacy Refund" : "General"),
...
export function computeFinanceMetrics(transactions = []) {
  ...
  transactions.forEach(tx => {
    ...
    if (isRefund) {
      totalRefund += amt;
      totalIncome -= amt;
      if (cat.includes("pharmacy") || title.includes("pharmacy") || cat === "pharmacy") {
        pharmacyIncome -= amt;
      }
