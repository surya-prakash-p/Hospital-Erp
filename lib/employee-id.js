...
/**
 * Normalizes common typos in Employee IDs (e.g. converting letter 'O'/'o' after TH to digit '0')
 */
export function normalizeEmployeeId(idStr) {
  if (!idStr || typeof idStr !== 'string') return '';
  const trimmed = idStr.trim();
  if (/^TH/i.test(trimmed)) {
    const prefix = trimmed.substring(0, 2);
    const rest = trimmed.substring(2).replace(/o/gi, '0');
...
export function isEmployeeIdUnique(employeeId, usersList = [], excludeUserId = null) {
  ...
  return !usersList.some(u => {
    ...
    const existingEmpId = normalizeEmployeeId(u.employeeId || u.employee_id || u.frappeStaffId || '');
