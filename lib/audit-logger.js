...
const LOCAL_LOGS_PATH = path.join(process.cwd(), 'thangam_hospital_audit_logs.json');
const TMP_LOGS_PATH = path.join(os.tmpdir(), 'thangam_hospital_audit_logs.json');
const SERVER_LOGS_PATH = path.join(os.tmpdir(), 'server_audit_logs.json');

// In-memory cache with timestamp
let memoryLogsCache = null;
let lastCacheReadTime = 0;
const CACHE_VALIDITY_MS = 2000;
...
function readLogsFromDisk() {
  const now = Date.now();
  if (memoryLogsCache && Array.isArray(memoryLogsCache) && memoryLogsCache.length > 0 && (now - lastCacheReadTime < CACHE_VALIDITY_MS)) {
...
/**
 * Writes logs safely to all disk targets
 */
