...
const INITIAL_MOCK_REGISTER = [
  { patient_name: "Surya Prakash", patient_id: "9876543210", doctor: "Dr. Rajesh", medicine: "Paracetamol 650mg", drug_category: "Regular Medicine", batch_number: "PM-EXP30D", quantity: 10, invoice_number: "INV-PH-10023", dispensing_date: "2026-07-27T10:15:00.000Z", pharmacist: "Rahul Sharma, RPh" },
  { patient_name: "Yokesh Raj", patient_id: "9876501234", doctor: "Dr. Priya", medicine: "Alprazolam 0.5mg", drug_category: "Sleeping Pill", batch_number: "AL-EXP3M", quantity: 5, invoice_number: "INV-PH-10024", dispensing_date: "2026-07-28T08:30:00.000Z", pharmacist: "Rahul Sharma, RPh" },
  { patient_name: "Surya Prakash", patient_id: "9876543210", doctor: "Dr. Vignesh", medicine: "Amoxicillin 500mg", drug_category: "Schedule H", batch_number: "AM-EXP30D", quantity: 15, invoice_number: "INV-PH-10025", dispensing_date: "2026-07-28T09:45:00.000Z", pharmacist: "Rahul Sharma, RPh" }
];

const INITIAL_MOCK_POS = [
  { name: "PO-2026-00001", supplier: "ABC Pharma", date: "2026-07-26", total_amount: 5400.0, status: "Submitted", items: [{ medicine: "Paracetamol 650mg", quantity: 300, purchase_price: 18.0, amount: 5400.0 }] }
];

const INITIAL_MOCK_GRNS = [
  { name: "GRN-2026-00001", purchase_order: "PO-2026-00001", supplier: "ABC Pharma", receipt_date: "2026-07-26", status: "Completed", items: [{ medicine: "Paracetamol 650mg", batch_number: "PM-STABLE", mfg_date: "2025-05-01", exp_date: "2027-05-01", quantity: 60, purchase_price: 18.0, selling_price: 20.0, rack_location: "Rack A-02" }] }
];
...
const INITIAL_MOCK_QUEUE = [
  {
    ...
    diagnosis: "Fever and suspected throat infection",
    prescription: "Paracetamol 650mg x 10 tablets (1-0-1)\nAmoxicillin 500mg x 15 capsules (1-1-1)\nAlprazolam 0.5mg x 5 tablets (0-0-1 sleeping pill at night)",
    need_lab_test: 0,
    lab_test_name: "",
...
function initLocalStorage() {
  ...
  if (!localStorage.getItem('hospital_batches')) {
    localStorage.setItem('hospital_batches', JSON.stringify(INITIAL_MOCK_BATCHES));
  }
  if (!localStorage.getItem('hospital_drug_register')) {
    localStorage.setItem('hospital_drug_register', JSON.stringify(INITIAL_MOCK_REGISTER));
  }
  if (!localStorage.getItem('hospital_purchase_orders')) {
    localStorage.setItem('hospital_purchase_orders', JSON.stringify(INITIAL_MOCK_POS));
  }
  if (!localStorage.getItem('hospital_goods_receipts')) {
    localStorage.setItem('hospital_goods_receipts', JSON.stringify(INITIAL_MOCK_GRNS));
  }
}
...
export async function getDbMode() {
  // Try to ping server to confirm connection, fallback dynamically
...
const INITIAL_MOCK_STAFF_USERS = [
  {
    ...
    status: "Active",
    permissions: ["Full System Access", "Manage Roles & Staff", "View Financials", "Manage Invoices", "Prescriptions", "Pharmacy Stock"]
  },
  {
    id: "USR-002",
  ...
  },
  {
    ...
    status: "Active",
    permissions: ["Dispense Medicines", "Manage Pharmacy Stock", "View Prescriptions", "Drug Register Access"]
  },
  {
    id: "USR-004",
    ...
    status: "Active",
    permissions: ["Register Patients", "Book Walk-Ins", "Collect OPD Fees", "View Doctor Schedules"]
  },
  {
    id: "USR-005",
    ...
    status: "Active",
    permissions: ["Enter Lab Results", "Upload Scan Reports", "Manage Test Catalog"]
  },
  {
    id: "USR-006",
    full_name: "Nurse Sarah",
    email: "sarah@thangamhospital.org",
    mobile_no: "9876543230",
    roles: ["Nurse"],
    department: "ICU & OPD Ward",
    designation: "Head Nurse",
    status: "Active",
    permissions: ["Patient Vitals", "View Patient Records", "Assist Consultations"]
  }
];
...
export async function getStaffUsers() {
  ...
  let localStaff = [];
  try {
    const stored = localStorage.getItem('hospital_staff_users');
    localStaff = stored ? JSON.parse(stored) : INITIAL_MOCK_STAFF_USERS;
  } catch (e) {
    localStaff = INITIAL_MOCK_STAFF_USERS;
  }

  // Also include doctors from hospital_doctors registry as staff members
  try {
    const docs = await getDoctors();
    docs.forEach(doc => {
      const docEmail = doc.email || `${doc.doctor_name?.toLowerCase().replace(/[^a-z0-9]/g, '')}@thangamhospital.org`;
      if (!localStaff.find(s => (s.email && s.email.toLowerCase() === docEmail.toLowerCase()) || (s.full_name && s.full_name.toLowerCase() === doc.doctor_name?.toLowerCase()))) {
        localStaff.push({
          id: `DOC-${Math.floor(100 + Math.random() * 900)}`,
          full_name: doc.doctor_name,
          email: docEmail,
          mobile_no: doc.mobile_number || docEmail,
          roles: ["Doctor"],
          department: doc.specialization || "Clinical Services",
    ...
    console.warn("Error merging doctors into staff list:", err);
  }

  if (typeof window !== 'undefined') {
    localStorage.setItem('hospital_staff_users', JSON.stringify(localStaff));
  }
...
export async function createStaffUser(userData) {
  ...
  // 1. Sync User credentials with backend API / Frappe
  try {
    await fetch('/api/users/manage', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.warn("Backend user creation warning:", e);
  }
  ...
  const newStaffMember = {
    ...
    permissions: userData.permissions || ["View Patient Records"]
  };

  const updatedStaff = [...staff.filter(s => s.email !== userData.email), newStaffMember];
  if (typeof window !== 'undefined') {
    localStorage.setItem('hospital_staff_users', JSON.stringify(updatedStaff));
  }
...
export async function updateUserRolesAndPermissions(email, newRoles, newPermissions) {
  initLocalStorage();
  const staff = await getStaffUsers();
  const target = staff.find(s => s.email.toLowerCase() === email.toLowerCase());

  if (!target) {
    throw new Error(`Staff user ${email} not found.`);
  }

  target.roles = newRoles;
  if (newPermissions) target.permissions = newPermissions;

  // Sync with Frappe User Backend
  try {
    await fetch('/api/users/manage', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.warn("Frappe user role sync warning:", e);
  }

  if (typeof window !== 'undefined') {
    localStorage.setItem('hospital_staff_users', JSON.stringify(staff));
  }
