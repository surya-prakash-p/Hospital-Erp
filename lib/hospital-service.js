...
function initLocalStorage() {
  ...
  // Database optimization: Migrate heavy base64 image strings out of main hospital_queue to keep operations lightweight
  try {
    const queueRaw = localStorage.getItem('hospital_queue');
    if (queueRaw && queueRaw.includes('data:image')) {
      const queueObj = JSON.parse(queueRaw);
      let migrated = false;
      const cleanedQueue = queueObj.map(item => {
        if (item.lab_test_image && item.lab_test_image.length > 200) {
          localStorage.setItem(`hospital_scan_images_${item.name}`, item.lab_test_image);
          item.lab_test_image = "stored_locally";
          migrated = true;
        }
        return item;
      });
      if (migrated) {
        localStorage.setItem('hospital_queue', JSON.stringify(cleanedQueue));
        console.log("Successfully migrated heavy image reports out of the main queue index!");
      }
    }
  } catch (err) {
...
// Fetch a single patient by Document Name, Patient ID, Mobile Number, or Name
export async function getPatient(identifier) {
  if (!identifier) return null;
  initLocalStorage();
  
  const cleanId = String(identifier).trim();
  const digitsOnly = cleanId.replace(/\D/g, ''); // Extract 10-digit mobile number if embedded in PAT-9876543210
  ...
  if (mode === 'frappe') {
    try {
      ...
      const filterStr = JSON.stringify([["Hospital Patient", "patient_name", "like", `%${cleanId}%`]]);
      ...
      console.warn(`Frappe getPatient ${cleanId} failed, fallback to local search`, e);
    }
  }
  ...
  if (mode === 'frappe') {
    try {
      ...
      if (res.ok) {
        ...
        // Merge Frappe data with local data to preserve frontend-only fields like next_checkup_date, dispensed_medicines, pharmacy_bill_amount
...
export async function createWalkIn(walkInData) {
  ...
  const recordConsultationPayment = (walkIn) => {
    if (walkIn.appointment_status === "Doctor Consultation") {
      ...
      // 2. Also record in finance ledger
      ...
      // 3. Save consultation invoice to patient profile documents
...
export async function updateWalkIn(name, updateData) {
  ...
  // Performance optimization: Extract large base64 image strings from queue updates to keep index extremely light
...
export async function askRAGAssistant(query, context = "", patientMobile = null) {
  ...
  return {
    answer: "Unable to reach RAG AI Assistant at the moment.",
    sources: []
  };
}

// Save invoice/document to patient profile documents
