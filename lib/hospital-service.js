...
function initLocalStorage() {
  ...
  // Clear legacy mock doctors if present
  const storedDocs = localStorage.getItem('hospital_doctors');
  if (!storedDocs || storedDocs.includes('Dr. Rajesh')) {
    localStorage.setItem('hospital_doctors', JSON.stringify([]));
  }

  // Clear legacy mock staff users if present
  const storedStaff = localStorage.getItem('hospital_staff_users');
  if (!storedStaff || storedStaff.includes('admin@thangamhospital.com') || storedStaff.includes('naughtyprakash')) {
    localStorage.setItem('hospital_staff_users', JSON.stringify(INITIAL_MOCK_STAFF_USERS));
  }

  // Clear legacy mock patients & queues
  const storedPatients = localStorage.getItem('hospital_patients');
  if (!storedPatients || storedPatients.includes('9876543210')) {
    localStorage.setItem('hospital_patients', JSON.stringify({}));
  }

  const storedQueue = localStorage.getItem('hospital_queue');
  if (!storedQueue || storedQueue.includes('HOSP-WALK-2026-00001')) {
    localStorage.setItem('hospital_queue', JSON.stringify([]));
  }

  const storedRegister = localStorage.getItem('hospital_drug_register');
  if (!storedRegister || storedRegister.includes('INV-PH-10023')) {
