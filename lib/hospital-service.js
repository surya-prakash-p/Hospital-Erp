  ...
  try {
    ...
    fetch('/api/logs/record', {
      ...
      body: JSON.stringify({
        ...
        actor: sessionUser ? {
          employeeId: sessionUser.employeeId || sessionUser.id || 'STAFF',
          name: sessionUser.full_name || sessionUser.name || 'Staff Member',
          role: sessionUser.role || sessionUser.roles?.[0] || 'Staff Member',
...
export async function createDoctor(doctorData) {
  ...
  if (doctorData.email && !doctorData.skipUserSync) {
    try {
...
export async function createPatient(patientData) {
  ...
  recordClientAudit(
    'patient',
    'Patient Registered',
    `Patient registered: ${patientData.patient_name} (${patientData.mobile_number})`,
    patientData.patient_name,
    { mobile: patientData.mobile_number, age: patientData.age, gender: patientData.gender }
  );
...
export async function createWalkIn(walkInData) {
  ...
  recordClientAudit(
    ...
    { doctor: walkInData.doctor, mobile: walkInData.mobile_number }
  );
...
export async function getMedicines() {
  ...
  const aggregatedMeds = medsRaw.map(med => {
    const medNameLower = (med.medicine_name || "").toLowerCase().trim();
    ...
    const matchedBatchesFromRaw = batchesRaw.filter(b => {
      ...
      return bMed === medNameLower || bMedName === medNameLower || b.medicine === medId;
    });
    ...
    const finalStock = totalStock > 0 ? totalStock : (med.stock || 0);
...
export async function updateDoctor(doctorName, doctorData) {
  ...
  // Sync User Credentials & Mobile Number if email is present
  if (doctorData.email) {
    try {
      await fetch('/api/users/manage', {
        method: 'POST',
