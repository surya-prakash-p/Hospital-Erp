...
export async function getDbMode() {
  ...
  const forceMode = localStorage.getItem('hospital_db_mode');
  if (forceMode) return forceMode; // 'frappe' or 'simulator'
  
  const connected = await checkConnection();
  const mode = connected ? 'frappe' : 'simulator';
  localStorage.setItem('hospital_db_mode', mode);
  return mode;
}
