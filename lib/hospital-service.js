...
export async function getStaffUsers() {
  ...
  // Fetch central cloud users for multi-device sync
  if (typeof window !== 'undefined') {
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && Array.isArray(data.users) && data.users.length > 0) {
          ...
          cloudUsers.forEach(cu => {
            ...
            const cuMobile = (cu.mobile_no || '').replace(/\D/g, '');

            const idx = localStaff.findIndex(ls => {
              const lsEmail = (ls.email || '').toLowerCase();
              const lsMobile = (ls.mobile_no || '').replace(/\D/g, '');
              return (cuEmail && lsEmail && cuEmail === lsEmail) || (cuMobile && lsMobile && cuMobile.length >= 7 && cuMobile === lsMobile);
            });

            if (idx !== -1) {
              localStaff[idx] = { ...localStaff[idx], ...cu };
            } else {
              localStaff.push(cu);
            }
          });
      ...
      console.warn("Cloud staff sync warning:", err);
    }
  }
...
export async function createStaffUser(userData) {
  ...
  try {
    ...
    if (!manageRes.ok) {
      const errData = await manageRes.json();
      console.warn("Backend user creation warning:", errData.error || manageRes.statusText);
    }
  } catch (e) {
  ...
  const existingStaff = staff.find(s => 
    ...
    (s.mobile_no && cleanMobile && s.mobile_no === cleanMobile)
  );

  const newStaffMember = {
    id: userData.id || (existingStaff ? existingStaff.id : `USR-${Math.floor(100 + Math.random() * 900)}`),
    full_name: userData.full_name || (existingStaff ? existingStaff.full_name : ''),
    email: cleanEmail || (existingStaff ? existingStaff.email : ''),
    mobile_no: cleanMobile || (existingStaff ? existingStaff.mobile_no : ''),
    password: cleanPassword || (existingStaff ? existingStaff.password : ''),
    roles: userData.roles || (existingStaff ? existingStaff.roles : [userData.role || 'Staff Member']),
    department: userData.department || (existingStaff ? existingStaff.department : 'Clinical'),
    designation: userData.designation || userData.role || (existingStaff ? existingStaff.designation : 'Staff'),
    status: userData.status || (existingStaff ? existingStaff.status : 'Active'),
    permissions: userData.permissions || (existingStaff ? existingStaff.permissions : ["View Patient Records"])
  };

  const updatedStaff = [...staff.filter(s => s.email !== newStaffMember.email && s.mobile_no !== newStaffMember.mobile_no), newStaffMember];
  if (typeof window !== 'undefined') {
...
export async function deleteStaffUser(identifier) {
  ...
  const target = staff.find(s => {
    ...
    const sMobileDigits = (s.mobile_no || s.phone || '').replace(/\D/g, '');

    const emailMatch = Boolean(sEmail && sEmail === cleanInput);
    const mobileMatch = Boolean(isDigits && sMobileDigits.length >= 7 && (
      sMobileDigits === cleanDigits || sMobileDigits.endsWith(cleanDigits) || cleanDigits.endsWith(sMobileDigits)
    ));
  ...
  if (typeof window !== 'undefined') {
    ...
    // Save to deleted users blacklist in localStorage
    try {
      const rawDel = localStorage.getItem('hospital_deleted_users');
      const delList = rawDel ? JSON.parse(rawDel) : [];
      if (cleanInput) delList.push(cleanInput);
      if (cleanDigits) delList.push(cleanDigits);
      if (target?.email) delList.push(target.email.toLowerCase());
      if (target?.mobile_no) delList.push(target.mobile_no.replace(/\D/g, ''));
      localStorage.setItem('hospital_deleted_users', JSON.stringify(Array.from(new Set(delList))));
    } catch (e) {}
  }

  // Delete from server backend credentials store & Frappe
  try {
    const param = target?.email || target?.mobile_no || identifier;
    await fetch(`/api/users/manage?identifier=${encodeURIComponent(param)}`, {
      method: 'DELETE'
    });
  } catch (err) {
    console.warn("Backend staff deletion sync warning:", err);
  }

  return target || { email: identifier };
}
