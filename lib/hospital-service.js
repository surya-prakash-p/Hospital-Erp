...
export async function getMedicines() {
  ...
  const aggregatedMeds = medsRaw.map(med => {
    // Find all batches for this medicine & normalize batch fields
    const medBatches = batchesRaw.filter(b => b.medicine === med.medicine_name).map(b => {
      const pSize = parseInt(b.pack_size) || 10;
      const cStock = (b.current_stock !== undefined && b.current_stock !== null) ? Number(b.current_stock) : Number(b.total_units || 0);
      const nPacks = (b.no_of_packs !== undefined && b.no_of_packs !== null) ? Number(b.no_of_packs) : (pSize > 0 ? (cStock / pSize) : cStock);
      const tUnits = pSize * nPacks;
      return {
        ...b,
        pack_size: pSize,
        no_of_packs: nPacks,
        ...
        mrp: b.mrp ?? b.selling_price ?? 0.0,
        supplier: b.supplier || "Default Supplier"
      };
    });
    // Sum active stock across batches
