...
export function recordClientAudit(type, action, description, target, metadata = {}) {
  ...
  try {
    ...
    .then(r => r.json())
    .then(data => {
      if (data?.success && data?.log && typeof window !== 'undefined') {
        try {
          const cached = JSON.parse(localStorage.getItem('hospital_audit_logs_cache') || '[]');
          if (Array.isArray(cached)) {
            const updated = [data.log, ...cached.filter(l => l.id !== data.log.id)].slice(0, 1000);
            localStorage.setItem('hospital_audit_logs_cache', JSON.stringify(updated));
          }
        } catch (e) {}
      }
    })
