...
export async function executeSalesReturn(returnData) {
  ...
  for (const item of (items || [])) {
    ...
    if (sold_qty !== undefined && Number(sold_qty) > 0) {
    ...
    if (restock_inventory) {
      ...
      await createStockMovementLog({
        ...
        remarks: `Return ID: ${returnId} | Orig Invoice: ${invoice_number || "N/A"} | Patient: ${patient_name || 'Customer'}`,
    ...
    await createDrugRegisterEntry({
      ...
      invoice_number: `${returnId} (Ref: ${invoice_number || 'N/A'})`,
